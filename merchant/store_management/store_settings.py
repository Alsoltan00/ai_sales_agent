...
def get_store_settings(client_id: str) -> dict:
    ...
    try:
        res = supabase.table("clients").select("company_name, contact_number, email, store_url, logo_url").eq("id", client_id).single().execute()
...
def update_store_settings(client_id: str, data: dict) -> bool:
    ...
    try:
        ...
        allowed_fields = ["company_name", "contact_number", "email", "store_url", "logo_url"]
