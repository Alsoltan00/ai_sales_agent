        ...
        if r_res.data and r_res.data.get("rules_data"):
            ...
            if checkout_type == "chat":
                ...
                if cart_behavior == "close_fast":
                    order_rule = "بمجرد أن يحدد العميل طلبه، احسب الإجمالي واطلب فوراً (الاسم والعنوان) لإنهاء الطلب بسرعة، ولا تسأله إن كان يريد شيئاً آخر."
                else:
                    order_rule = (
                        "يجب إتمام الطلب على خطوتين:\n"
                        "1. بعد أن يطلب العميل منتجاً، احسب الإجمالي واسأله أولاً: 'هل ترغب بإضافة شيء آخر للطلب؟'.\n"
                        "2. يُمنع منعاً باتاً طلب (الاسم والعنوان) إلا بعد أن يؤكد العميل أنه لا يريد إضافة شيء آخر."
                    )
                
                if confirm_type == "summary":
                    order_rule += "\n- قبل الاعتماد النهائي، أرسل ملخصاً صريحاً للفاتورة واطلب موافقته."
            else:
