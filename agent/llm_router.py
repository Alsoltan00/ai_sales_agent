...
def classify_intent_and_extract_keywords(user_message: str, chat_history: list, store_name: str = "المتجر", store_id: str = None, system_prompt_custom: str = None) -> dict:
    ...
    مهمتك تحليل رسالة العميل (مع فهم سياق المحادثة السابقة بدقة شديدة) وإخراج رد بصيغة JSON فقط، يحتوي على المفاتيح التالية:
    
    1. "action": 
       - اختر "search": فقط إذا كان العميل يسأل عن منتج للبحث عنه في قاعدة البيانات (مثل: وش عندكم عطور، هل متوفر سيروم الوجه؟).
       - اختر "chat": إذا كان العميل يقوم بأحد الأمور التالية:
           * يرحب بك (مثل: السلام عليكم).
           * يختار منتجاً من قائمة عرضتها له مسبقاً (مثل: "أبي رقم 1"، "عطني الأول").
           * يحدد كمية لمنتج تحدثتم عنه للتو (مثل: "عطني 5 حبات فقط").
           * يكمل بيانات الطلب (الاسم، الموقع).
           في هذه الحالات، أنت لا تحتاج للبحث من جديد لأن المنتج معروف من المحادثة السابقة.
       
    2. "reply": 
       - إذا كان الخيار "chat"، اكتب ردك النهائي المناسب للعميل بأسلوب لبق وتفاعلي. مثلاً إذا قال "عطني 5 حبات" لمنتج سابق، قل: "أبشر طال عمرك، جهزت لك 5 حبات من (اسم المنتج). بس عطني الاسم ورقم الجوال عشان أعتمد طلبك."
       - إذا كان الخيار "search" اتركه فارغاً "".
       
    3. "keywords": 
       - إذا كان "search"، استخرج الاسم الجوهري للبحث. إذا كان العميل يشير لمنتج ذُكر سابقاً، استخدم اسمه الفعلي من المحادثة وليس الإشارة (مثلاً "سيروم لوريال" بدلاً من "الأول").
    """
...
def generate_sales_reply(user_message: str, filtered_products: list, chat_history: list = None, store_name: str = "المتجر", system_prompt_custom: str = None) -> str:
    ...
    5. الالتزام والهلوسة: لا تخترع معلومات أو منتجات غير موجودة في البيانات المرفقة أبداً. لا ترد على أسئلة سابقة تمت الإجابة عليها بالفعل، ركز فقط على الرسالة الأخيرة للعميل.
    """
